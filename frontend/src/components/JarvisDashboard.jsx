...
export default function JarvisDashboard({ currentUser, onLogout }) {
  ...
  async function submitCommand(text) {
    ...
    try {
      ...
      addJarvisResponse(data.response, data.source);
      setApiOnline(true);
    } catch {
      setApiOnline(false);
      addMessage(
  ...
  function addJarvisResponse(text, source) {
    addMessage("jarvis", text, source);
    speakInBrowser(text);
  }

  function speakInBrowser(text) {
    if (!voiceEnabled || !browserCanSpeak || !text) {
      return;
    }
        ...
        <aside className="flex w-[290px] flex-col gap-4 overflow-y-auto border-r border-cyan-500/10 bg-[#05101b]/40 p-4 backdrop-blur-lg">
          ...
          <GlassCard title="Comandos">
            <div className="grid gap-2">
              {["abrir instagram", "abrir youtube", "abrir calculadora"].map(
                (command) => (
                  <button
